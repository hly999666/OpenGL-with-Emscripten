...

{
	...
	lyh_gl::shading::Shader pbr_shader("c45_PBR_ibl_pbr_obj.glsl");
	lyh_gl::shading::Shader equirectangularToCubemapShader("c45_PBR_ibl_skybox.glsl");
	lyh_gl::shading::Shader irradianceShader("c45_PBR_ibl_irradiance_convolution.glsl");
	lyh_gl::shading::Shader backgroundShader("c45_PBR_ibl_background.glsl");

	//setup shader
	pbr_shader.use();

	pbr_shader.setInt("irradianceMap", 0);
	pbr_shader.setVec3("albedo", 0.5f, 0.0f, 0.0f);
	...
	float* data = stbi_loadf("./res/texture/newport_loft.hdr", &img_width, &img_height, &nrComponents, 0);
	unsigned int hdrTexture;
	if (data)
	{
		glGenTextures(1, &hdrTexture);
		glBindTexture(GL_TEXTURE_2D, hdrTexture);
		//note type is GL_FLOAT
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, img_width, img_height, 0, GL_RGB, GL_FLOAT, data);
	...
	//render cube map
	{
		//set up shader for convert to cubemap
	...
	//convolution cubemap
	{
		...
		//configure the viewport to the capture dimensions,with is much small
	...
	render_loop = [&] {
		 ...
		 //imGui
		{
			...
			ImGui::SetNextWindowSize({ 240,96 });
			ImGui::Begin("Render Input");
			ImGui::Checkbox("Background is convoluted", &background_is_blur);
			ImGui::Checkbox("Using diffuse irradiance map", &irradiance_map);
		...
		{
			glm::mat4 model = glm::mat4(1.0f);
			pbr_shader.setInt("ambient_mode", (int)irradiance_map);
