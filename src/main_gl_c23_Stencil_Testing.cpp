...

{
	...
	lyh_gl::shading::Shader stencil_outline_shader("lighting_c23_stencil_testing_outline.glsl");
	...
	float col_light[4] = { 0.5,0.5,0.5,1.0 };
	  ...
	  float col_line[4] = { 0.5,0.5,0.5,1.0 };
	...
	render_loop = [&] {
		...
		//enable global stencil test
		glEnable(GL_STENCIL_TEST);
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
		//must set third as GL_REPLACE,otherwise stencil will not change
		...
		ImGui::Begin("Outline Setting");
		 
		ImGui::ColorPicker4("outline_color", col_line);
		ImGui::SliderFloat("outline_width", &line_width,0.1,0.5);
		...
		//draw plane first without write to stencil
		glStencilMask(0x00);
		...
		//first pass,draw will stencil written
		...
		//second pass 
		//draw only where stencil unwritten
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
		//stencil is read-only
		glStencilMask(0x00);
		//disable depth test for outline effect
		glDisable(GL_DEPTH_TEST);

		float line_width_scale = 3.0 + line_width;
		stencil_outline_shader.use();
		stencil_outline_shader.setMat4("projection", projection);
		stencil_outline_shader.setMat4("view", view);
		stencil_outline_shader.setVec4("line_colur",glm::vec4(col_line[0], col_line[1], col_line[2], col_line[3]));
		glBindVertexArray(VAO);
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(0.0, 0.0, 0.0));
		model = glm::scale(model, glm::vec3(line_width_scale));
		stencil_outline_shader.setMat4("model", model);
		//stencil_outline_shader.setVec2("uvScale", 1.0f, 1.0f);
		//stencil_outline_shader.bindTex("diffuse_map", *metal_diffuse);
		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(5.0, 0.0, -6.0));
		model = glm::scale(model, glm::vec3(line_width_scale)); // Make it a smaller cube
		stencil_outline_shader.setMat4("model", model);
		//stencil_outline_shader.setVec2("uvScale", 1.0f, 1.0f);
		//stencil_outline_shader.bindTex("diffuse_map", *metal_diffuse);
		glDrawArrays(GL_TRIANGLES, 0, 36);
