...
#include<vector>
void fractalTree2D(double theta, double s, std::vector<float>& x, std::vector<float>& y,glm::mat4 current_mat, double len) {
    using namespace glm;
    ...
    fractalTree2D(theta, s, x, y, m_t *   m_rl * m_s * current_mat, len);
    fractalTree2D(theta, s, x, y, m_t *  m_rr * m_s * current_mat, len);
}
namespace DDoN {
    void fractalTree2D_Basic(double theta, double s, std::vector<float>& x, std::vector<float>& y,double len=5.0) {
        fractalTree2D(theta,s,x,y,glm::mat4(1.0),len);
    }
